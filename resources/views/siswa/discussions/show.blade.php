@extends('layouts.app', ['title' => 'Ruang Obrolan'])
...
<div class="flex">
    ...
    <div class="flex-1 p-6 max-w-4xl">
        <a href="{{ route('siswa.discussions.index') }}" class="text-sm font-semibold text-blue-600 hover:underline">← Kembali ke Forum</a>
        ...
        <div class="space-y-4 mb-6">
            ...
            @empty
                <p class="text-gray-400 italic ml-6">Belum ada tanggapan. Bantu temanmu menjawab pertanyaan ini!</p>
            @endforelse
        </div>

        <div class="bg-white border rounded-xl p-5 shadow-sm ml-6">
            <h4 class="font-bold text-gray-800 mb-3 text-sm">Ikut Berdiskusi:</h4>
            <form action="{{ route('siswa.discussions.store_reply', $topic->id) }}" method="POST" class="space-y-3">
                @csrf
                <textarea name="content" rows="3" required placeholder="Tulis komentar atau bantuan jawaban kamu di sini..." class="w-full border rounded-lg px-3 py-2 bg-gray-50 focus:ring-2 focus:ring-blue-500 outline-none text-sm"></textarea>
